...
%     end
    ...
    %watershedImage = watershed(imcomplement((bwdist(labelImage==0)))) > 0;
    watershedImage = watershed(imcomplement(dogImage)) > 0;
    labelImage = labelImage .* watershedImage;
    ...
    %         headSearchRadius = segmentRadius;
    ...
    %         headSearchRegion = dogImage .* (labelImage(rangeY, rangeX) == r);
            [~, maxIndices] = max(dogImage(currentRegionProps(r).PixelIdxList));
            [xpos, ypos] = ind2sub(size(labelImage), currentRegionProps(r).PixelIdxList(maxIndices));
            %correctedHeadLocation = [initialHeadLocation(1) + (ypos - ((length(rangeY)-1)/2) - 1), initialHeadLocation(2) + (xpos - ((length(rangeX)-1)/2) - 1)];
            correctedHeadLocation = [ypos, xpos];
...
%                 maxRegionIntensity = max(max(rawImage(rangeY, rangeX)));
                ...
                if (maxValue > 0)
                    larvaModel = [larvaModel; nextPoint]; %#ok<AGROW>
                    if (i>1)
                        directionAngles = [directionAngles; maxAngle]; %#ok<AGROW>
                    end
            ...
            if (validLarva == true)
                ...
                for i=2:size(larvaModel,1)
                    ...
                    if i<size(larvaModel,1)
                        ...
                        relativeAngles = [relativeAngles; rad2deg(real(acos(dot((nextDirection/norm(nextDirection)), (currentDirection/norm(currentDirection))))))]; %#ok<AGROW>
                    end
                    ...
                    directionAngles = [directionAngles; rad2deg(acos(dot(currentDirection, [1,0])))]; %#ok<AGROW>
                end
